# -*- coding: utf-8 -*-
"""
Created on Wed Feb 16 16:52:32 2022

@author: Robbe Decapmaker
"""

from sympy import *
from sympy.plotting import *
import numpy as np
import matplotlib.pyplot as plt

xx = np.linspace(-3,9,100)
plt.figure()
x = symbols('x', real = true)
plt.grid(True, which='both')

plt.axhline(y=0, color='k')
plt.axvline(x=0, color='k')
plt.ylabel("Verplaatsing (m)")
plt.xlabel("\u03C9 t (s)")

def f(x): return np.cos(x + np.pi/2)


plt.plot(xx,f(xx),'r', lw = 1)
plt.hlines(1,-3,9)
plt.hlines(-1,-3,9)
plt.axvline(x=-np.pi/2)
plt.yticks([])
plt.text(-np.pi/2 +0.1, 0.1, '- \u03C6')
plt.text(-2.8, 0.9, 'A')
plt.text(-2.8, -0.95, '- A')
#plt.show()
plt.savefig('Harmonische_Oscillator.pdf')

plt.clf()
xx = np.linspace(-1,1,100)
def f(x) : return x**2
def g(x) : return -x**2 + 1

Potential = plt.plot(xx,f(xx),'r', lw = 1)
Kinetic = plt.plot(xx,g(xx),'g', lw = 1)
plt.xlabel("Verplaatsing (m)")
plt.ylabel("E(x) (J)")
#plt.hlines(1,-1,1)
plt.legend(loc='lower left',labels=['Potentiële energie', 'Kinetische energie'])
#plt.show()
plt.savefig('Energie_Balans.pdf')

plt.clf()
xx = np.linspace(0,20,100)
def f(x) : return np.e**(-(0.3)*x)*np.sin(x)
def g(x) : return np.e**(-(0.3)*x)
def h(x) : return -np.e**(-(0.3)*x)
plt.plot(xx,f(xx),'r',lw=2)
plt.plot(xx,g(xx),'gray',lw=1,linestyle=':')
plt.plot(xx,h(xx),'gray',lw=1,linestyle=':')
plt.hlines(0,20,0,'k')
plt.ylabel("Verplaatsing (m)")
plt.xlabel("t (s)")
plt.yticks([])
plt.xticks([])
#plt.show()
plt.savefig('Ondergedempt.pdf')


plt.clf()
m = 1
k = 1
xx = np.linspace(0,3,100)
def f(x) : return np.e**(-(0.8)*x)*np.cos(0.5*x)
def g(x) : return np.e**(-(0.1)*x)*np.cos(0.5*x)
plt.plot(xx,f(xx),'r',lw=1)
plt.plot(xx,g(xx),'b',lw=1)
plt.hlines(0,3,0,'k')
plt.ylabel("Verplaatsing (m)")
plt.xlabel("t (s)")
plt.yticks([])
plt.xticks([])
plt.legend(loc='lower left',labels=['Kritisch gedempt', 'overgedempt'])
#plt.show()
plt.savefig('Over_Kritisch_gedempt.pdf')

plt.clf()
xx = np.linspace(0,12,100)
def f(x): return np.cos(x + np.pi/2)
def g(x): return np.cos(x + np.pi)
plt.plot(xx,f(xx),'r',lw=1)
plt.plot(xx,g(xx),'b',lw=1)
plt.yticks([])
plt.xticks([])
plt.hlines(0,12,0,'k')
plt.xlabel("Verplaatsing (m)")
plt.ylabel("Amplitude (m)")
plt.legend(loc='lower right',labels=['tijdstip 2', 'tijdstip 1'])
plt.axvline(x=np.pi, color='k', linestyle = ':')
plt.axvline(x=np.pi+np.pi/2, color='k', linestyle = ':')
plt.hlines(-0.5,np.pi+np.pi/2,np.pi,'k')
plt.text(np.pi+np.pi/6, -0.4, 'dx')
plt.savefig('ruimtelijke_analyse.pdf')

plt.clf()
xx = np.linspace(0,12,100)
def f(x): return np.cos(x + np.pi/2)
plt.plot(xx,f(xx),'r',lw=1)
plt.yticks([])
plt.xticks([])
plt.hlines(0,12,0,'k')
plt.xlabel("Verplaatsing (m)")
plt.ylabel("Amplitude (m)")
plt.axvline(x=3*np.pi+np.pi/2, color='k', linestyle = '-.')
plt.axvline(x=np.pi+np.pi/2, color='k', linestyle = '-.')
plt.hlines(0.7,3*np.pi+np.pi/2,np.pi+np.pi/2,'k')
plt.text(2*np.pi+3*np.pi/7, 0.8, 'λ')
plt.axvline(x=np.pi/2, ymin= 0.05, ymax = 0.5, color='k', linestyle = ':')
plt.axvline(x=np.pi+np.pi/6, ymin= 0.5, ymax = f(np.pi+np.pi/6)+0.2, color='k', linestyle = ':')
plt.text(np.pi/2+0.1, -0.1, 'x1')
plt.text(np.pi+np.pi/6+0.1, 0.05, 'x2')

plt.savefig('tijdsdomein_analyse.pdf')

plt.clf()
xx = np.linspace(0,12,100)
def f(x): return np.cos(x + np.pi/2)
def g(x): return np.cos(x + np.pi)
plt.plot(xx,f(xx),'r',lw=1)
plt.plot(xx,g(xx),'b',lw=1)
plt.yticks([])
plt.xticks([])
plt.hlines(0,12,0,'k')
plt.xlabel("Verplaatsing (m)")
plt.ylabel("Amplitude (m)")
plt.legend(loc='lower right',labels=['y 2', 'y 1'])

plt.savefig('Interferentie.pdf')

plt.clf()
xx = np.linspace(0,12,100)
def f(x): return np.cos(x)
def g(x): return np.cos(x + np.pi)
plt.plot(xx,f(xx),'r',lw=1)
plt.plot(xx,g(xx),'b',lw=1)
plt.yticks([])
plt.xticks([])
plt.hlines(0,12,0,'k')
plt.xlabel("Verplaatsing (m)")
plt.ylabel("Amplitude (m)")
plt.legend(loc='lower right',labels=['y 2', 'y 1'])

plt.savefig('Destructieve_interferentie.pdf')

plt.clf()
xx = np.linspace(0,12,100)
def f(x): return np.cos(x)
def g(x): return np.cos(x)
def h(x): return 2*np.cos(x)
plt.plot(xx,f(xx),'r',lw=1)
plt.plot(xx,g(xx),'b',lw=1)
plt.plot(xx,h(xx),'g',lw=1)
plt.yticks([])
plt.xticks([])
plt.hlines(0,12,0,'k')
plt.xlabel("Verplaatsing (m)")
plt.ylabel("Amplitude (m)")
plt.legend(loc='lower right',labels=['y 2', 'y 1', 'y 1 + y 2'])

plt.savefig('Constructieve_interferentie.pdf')

plt.clf()
xx = np.linspace(0,12,100)
def f(x): return np.sin(x*np.pi/12)
plt.plot(xx,f(xx),'r',lw=1)
plt.plot(xx,-f(xx),'r',lw=1, linestyle = ':')
plt.yticks([])
plt.xticks([])
plt.hlines(0,12,0,'k')
plt.axvline(x=0, color='k', linestyle = '-')
plt.axvline(x=12, color='k', linestyle = '-')
plt.xlabel("Positie (m)")
plt.ylabel("Amplitude ifv Positie (m)")

plt.savefig('Eerste_harm.pdf')

plt.clf()
xx = np.linspace(0,12,100)
def f(x): return np.sin(x*np.pi/6)
plt.plot(xx,f(xx),'r',lw=1)
plt.plot(xx,-f(xx),'r',lw=1, linestyle = ':')
plt.yticks([])
plt.xticks([])
plt.hlines(0,12,0,'k')
plt.axvline(x=0, color='k', linestyle = '-')
plt.axvline(x=12, color='k', linestyle = '-')
plt.xlabel("Positie (m)")
plt.ylabel("Amplitude ifv Positie (m)")

plt.savefig('Tweede_harm.pdf')

plt.clf()
xx = np.linspace(0,12,100)
def f(x): return np.sin(x*np.pi/4)
plt.plot(xx,f(xx),'r',lw=1)
plt.plot(xx,-f(xx),'r',lw=1, linestyle = ':')
plt.yticks([])
plt.xticks([])
plt.hlines(0,12,0,'k')
plt.axvline(x=0, color='k', linestyle = '-')
plt.axvline(x=12, color='k', linestyle = '-')
plt.xlabel("Positie (m)")
plt.ylabel("Amplitude ifv Positie (m)")

plt.savefig('Derde_harm.pdf')

plt.clf()
xx = np.linspace(0,np.pi*4,100)
def f(x): return np.sin(x)
plt.plot(xx,f(xx),'r',lw=1)
plt.yticks([])
plt.xticks([])
plt.xlabel("Positie (m)")
plt.ylabel("Amplitude (m)")
plt.hlines(0,np.pi*4,0,'k',linestyles=":")
plt.hlines(-1.2,np.pi*4,0,'k')
plt.axvline(x=np.pi*2-0.3, ymin= 0.4, ymax = 0.45, color='k', lw=6)
plt.axvline(x=np.pi*2-0.8, ymin= 0.3, ymax = 0.54, color='k', lw=1)
plt.axvline(x=np.pi*2+0.1, ymin= 0.3, ymax = 0.54, color='k', lw=1)
plt.hlines(0,np.pi*2+0.15,np.pi*2-0.8,'k')
plt.hlines(-0.6,np.pi*2+0.15,np.pi*2-0.8,'k')
plt.text(np.pi+np.pi-0.65, -0.15, 'dm')
plt.text(np.pi+np.pi+0.2, -0.32, 'S')
plt.text(np.pi+np.pi-0.60, -0.7, 'dx')
plt.savefig("EnergieGolf.pdf")

plt.clf()
xx = np.linspace(0,np.pi*4,100)
def f(x): return np.sin(x)
plt.plot(xx,f(xx),'r',lw=1)
plt.yticks([])
plt.xticks([])
plt.hlines(0,np.pi*4,0,'k')
plt.xlabel("Positie (m)")
plt.ylabel("Druk (Pa)")
plt.axvline(x=0, ymin= 0.02, ymax= 0.98, color='k', lw=1)
plt.savefig("Drukinbuis.pdf")

plt.clf()
xx = np.linspace(0,np.pi*4,100)
def f(x): return np.sin(x)
def g(x): return np.sin(x-np.pi/2)
plt.plot(xx,f(xx),'r',lw=1)
plt.plot(xx,g(xx),'g',lw=1)
plt.yticks([])
plt.xticks([])
plt.hlines(0,np.pi*4,0,'k')
plt.xlabel("Positie x (m)")
plt.ylabel("Druk (Pa) / Verplaatsing (m)")
plt.legend(loc='lower right',labels=['Verplaatsing', 'Druk'])
plt.axvline(x=0, ymin= 0.02, ymax= 0.98, color='k', lw=1)
plt.savefig("FaseVerschilDrukVerplaatsing.pdf")

plt.clf()
xx = np.linspace(0,12,100)
def f(x): return np.sin(x*np.pi/12)
plt.plot(xx,f(xx),'r',lw=1)
plt.plot(xx,-f(xx),'r',lw=1, linestyle = ':')
plt.yticks([])
plt.xticks([])
plt.hlines(0,12,0,'k', linestyles = ':')
plt.hlines(1,12,0, color='k', linestyle = '-')
plt.hlines(-1,0,12,'k')
plt.xlabel("Positie (m)")
plt.ylabel("Druk ifv Positie (Pa)")
plt.savefig("OpenBuisEersteDruk.pdf")

plt.clf()
xx = np.linspace(0,12,100)
def f(x): return np.sin(x*np.pi/12+np.pi/2)
plt.plot(xx,f(xx),'r',lw=1)
plt.plot(xx,-f(xx),'r',lw=1, linestyle = ':')
plt.yticks([])
plt.xticks([])
plt.hlines(0,12,0,'k', linestyles = ':')
plt.hlines(1,12,0, color='k', linestyle = '-')
plt.hlines(-1,0,12,'k')
plt.xlabel("Positie (m)")
plt.ylabel("Verplaatsing ifv Positie (m)")
plt.savefig("OpenBuisEersteVerplaatsing.pdf")

plt.clf()
xx = np.linspace(0,12,100)
def f(x): return np.sin(x*np.pi/6)
plt.plot(xx,f(xx),'r',lw=1)
plt.plot(xx,-f(xx),'r',lw=1, linestyle = ':')
plt.yticks([])
plt.xticks([])
plt.hlines(0,12,0,'k', linestyles = ':')
plt.hlines(1,12,0, color='k', linestyle = '-')
plt.hlines(-1,0,12,'k')
plt.xlabel("Positie (m)")
plt.ylabel("Druk ifv Positie (Pa)")
plt.savefig("OpenBuisTweedeDruk.pdf")

plt.clf()
xx = np.linspace(0,12,100)
def f(x): return np.sin(x*np.pi/6+np.pi/2)
plt.plot(xx,f(xx),'r',lw=1)
plt.plot(xx,-f(xx),'r',lw=1, linestyle = ':')
plt.yticks([])
plt.xticks([])
plt.hlines(0,12,0,'k', linestyles = ':')
plt.hlines(1,12,0, color='k', linestyle = '-')
plt.hlines(-1,0,12,'k')
plt.xlabel("Positie (m)")
plt.ylabel("Verplaatsing ifv Positie (m)")
plt.savefig("OpenBuisTweedeVerplaatsing.pdf")

plt.clf()
xx = np.linspace(0,12,100)
def f(x): return np.sin(x*np.pi/4)
plt.plot(xx,f(xx),'r',lw=1)
plt.plot(xx,-f(xx),'r',lw=1, linestyle = ':')
plt.yticks([])
plt.xticks([])
plt.hlines(0,12,0,'k', linestyles = ':')
plt.hlines(1,12,0, color='k', linestyle = '-')
plt.hlines(-1,0,12,'k')
plt.xlabel("Positie (m)")
plt.ylabel("Druk ifv Positie (Pa)")
plt.savefig("OpenBuisDerdeDruk.pdf")

plt.clf()
xx = np.linspace(0,12,100)
def f(x): return np.sin(x*np.pi/4+np.pi/2)
plt.plot(xx,f(xx),'r',lw=1)
plt.plot(xx,-f(xx),'r',lw=1, linestyle = ':')
plt.yticks([])
plt.xticks([])
plt.hlines(0,12,0,'k', linestyles = ':')
plt.hlines(1,12,0, color='k', linestyle = '-')
plt.hlines(-1,0,12,'k')
plt.xlabel("Positie (m)")
plt.ylabel("Verplaatsing ifv Positie (m)")
plt.savefig("OpenBuisDerdeVerplaatsing.pdf")

plt.clf()
xx = np.linspace(0,12,100)
def f(x): return np.sin(x*np.pi/24)
plt.plot(xx,f(xx),'r',lw=1)
plt.plot(xx,-f(xx),'r',lw=1, linestyle = ':')
plt.yticks([])
plt.xticks([])
plt.hlines(0,12,0,'k', linestyles = ':')
plt.hlines(1,12,0, color='k', linestyle = '-')
plt.hlines(-1,0,12,'k')
plt.axvline(12,0,1,color='k')
plt.xlabel("Positie (m)")
plt.ylabel("Druk ifv Positie (Pa)")
plt.savefig("HalfOpenBuisEersteDruk.pdf")

plt.clf()
xx = np.linspace(0,12,100)
def f(x): return np.sin(x*np.pi/24+np.pi/2)
plt.plot(xx,f(xx),'r',lw=1)
plt.plot(xx,-f(xx),'r',lw=1, linestyle = ':')
plt.yticks([])
plt.xticks([])
plt.hlines(0,12,0,'k', linestyles = ':')
plt.hlines(1,12,0, color='k', linestyle = '-')
plt.hlines(-1,0,12,'k')
plt.axvline(12,0,1,color='k')
plt.xlabel("Positie (m)")
plt.ylabel("Snelheid ifv Positie (m/s)")
plt.savefig("HalfOpenBuisEersteSnelheid.pdf")

plt.clf()
xx = np.linspace(0,np.pi*4,100)
def f(x): return np.sin(x)
plt.plot (xx,f(xx),'r',lw=1)
plt.yticks([])
plt.xticks([])
plt.hlines(0,np.pi*4,0,'k')


plt.axvline(x=np.pi*2-0.8, ymin= 0.045, ymax = 0.955, color='k', lw=1)
plt.axvline(x=np.pi*2+0.15, ymin= 0.045, ymax = 0.955, color='k', lw=1)
plt.hlines(-1,np.pi*2+0.15,np.pi*2-0.8,'k')
plt.hlines(1,np.pi*2+0.15,np.pi*2-0.8,'k')
plt.text(np.pi+np.pi-1, 0.03, 'h')
plt.text(np.pi+np.pi-0.50, 1.03, 'dx')

plt.xlabel("Positie (m)")
plt.ylabel("Electrisch veld E")
plt.savefig("FaradayGolf.pdf")

plt.clf()
xx = np.linspace(0,np.pi*4,100)
def f(x): return np.sin(x)
plt.plot (xx,f(xx),'r',lw=1)
plt.yticks([])
plt.xticks([])
plt.hlines(0,np.pi*4,0,'k')


plt.axvline(x=np.pi*2-0.8, ymin= 0.045, ymax = 0.955, color='k', lw=1)
plt.axvline(x=np.pi*2+0.15, ymin= 0.045, ymax = 0.955, color='k', lw=1)
plt.hlines(-1,np.pi*2+0.15,np.pi*2-0.8,'k')
plt.hlines(1,np.pi*2+0.15,np.pi*2-0.8,'k')
plt.text(np.pi+np.pi-1, 0.03, 'h')
plt.text(np.pi+np.pi-0.50, 1.03, 'dx')

plt.xlabel("Positie (m)")
plt.ylabel("Magnetisch veld B")
plt.savefig("MaxwellGolf.pdf")